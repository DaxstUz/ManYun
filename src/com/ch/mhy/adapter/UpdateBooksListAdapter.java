 ...
 */
public class UpdateBooksListAdapter extends BaseAdapter {
	private List<Comics> list;
	private Context content;

	private LayoutInflater inflater;

	public UpdateBooksListAdapter(Context content, List<Comics> list) {
		this.list = list;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		TextView tv_author_bookname = (TextView) view.findViewById(R.id.tv_author_bookname);
		tv_author_bookname.setText(list.get(position).getmTitle().length() > 10 ? list
		...
		String chapter = list.get(position).getIp();
		if(chapter.length()>16){
			chapter = chapter.substring(0, 16)+"...";
		}
		tv_author_renqi.setText("更新章节：" + chapter);
		...
		ImageLoader.getInstance().displayImage(imgUrl, iv_cate_authorshow,
				Utils.adapterOpt);
		return view;
	}
